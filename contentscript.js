...
function getElementType(otherelement) {
  var sRole = "";
  if (otherelement.getAttribute("role") !== null && otherelement.getAttribute("role") != "") {
    sRole = otherelement.getAttribute("role");
  }

  if ((sRole == "switch") || (otherelement.className == "toggle-box") || (otherelement.className == "toggle-value")) {
    //data-dyn-role="CheckBox"
    var checkBox = otherelement.closest('[data-dyn-role="CheckBox"]')
    var checkBoxName = checkBox.getElementsByClassName("staticText label checkBox-label").item(0)
    return "radio"
  }

  var parentAppBarSection = otherelement.closest('[role="grid"]');
  columnHeaderEle = null
  if (parentAppBarSection) {
    return "table"
  }

  var parentAppBarSection = otherelement.closest('[data-dyn-role = "AppBarSection"]');
  if (parentAppBarSection) {
    return "menu"
  }
  return "standard"
      ...
      if (elems[i].textContent == eleName) {
        ...
        if (rect.top == 0 && rect.right == 0 && rect.bottom == 0 && rect.left == 0) {
          ...
          if (visibleEles.length > 0) {
            ...
            if (!isDesc && incCount) {
              if(getElementType(elems[i])==eleType){
                count = count + 1
...
chrome.extension.sendMessage({ type: "get_project" }, function (response) {
  ...
  if (name != null) {
    ...
    console.log("customer and project added");
    var requestOptions = {
      mode: "no-cors",
      method: 'POST',
      body: formdata,
